...
confused_data <- OBA_df %>%
  filter(OBA_df$Species == "confusus") %>%
  select(Species, GenusSpecies, Dec..Lat., Dec..Long.,
         Associated.plant...genus..species, MonthJul)

colnames(confused_data) <- c("Species", "GenusSpecies", "Lat", "Long",
                           "Associated Plant", "Month")
confused_data$Month <- ifelse(confused_data$Month == "", "None Specified", confused_data$Month)
...
confused_sf <- st_as_sf(confused_data, coords = c("Long", "Lat"), crs = 4326)
...
ggplot() +
  ...
  theme(
    ...
    legend.title = element_text(size = 11), # Title 
    legend.text = element_text(size = 9) # Text 
  )
...
flowers_1000 <- st_intersection(cofused_sf, buffer_1000)
