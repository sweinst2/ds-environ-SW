...
#Removing lists were working earlier
#oba$Associated.plant <- oba[!grepl(",", oba$Associated.plant),]

#removing blanks these were working earlier 
...
for (i in 1:nrow(oba)) {
  if (grepl("aceae$", oba$Associated.plant[i], ignore.case = TRUE)) {
    ...
    oba$plantResolution[i] <- "Missing/Complex"
  }
}
...
oba$Associated.plant <- str_replace_all(oba$Associated.plant, "^([A-Za-z]+)$", "\\1 sp.")
...
for (i in 1:nrow(oba)) {
  ...
  words <- unlist(strsplit(oba$Associated.plant[i]," "))
  if (oba$plantResolution[i] == "Species" || oba$plantResolution[i] == "Genus") {
    oba$plantGenus[i] <- words[1]
  }
  else if (oba$plantResolution[i] == "Family" || oba$plantResolution[i] == "Missing/Complex") {
    oba$plantGenus[i] <- NA
  }
}
